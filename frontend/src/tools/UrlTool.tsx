...
export function UrlTool() {
  ...
  const { history, addHistory, clearHistory } = useHistory('url');
  ...
  const handleEncode = async () => {
    try {
      setError('');
      const result = await window.go.tools.ToolService.UrlEncode(input);
      setOutput(result);
  ...
  const handleDecode = async () => {
    try {
      setError('');
      const result = await window.go.tools.ToolService.UrlDecode(input);
      setOutput(result);
  ...
  return (
    <div className="h-full flex flex-col p-6 bg-white dark:bg-gray-900">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white">URL Encode/Decode</h2>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Encode or decode URL strings
            <span className="ml-2 text-xs text-gray-400 dark:text-gray-500">Cmd+E encode, Cmd+D decode</span>
          </p>
        </div>
        <button
        ...
        <div className={`flex-1 flex flex-col gap-4 ${showHistory ? 'w-2/3' : 'w-full'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              title="Use output as input (Cmd+Shift+S)"
            >
