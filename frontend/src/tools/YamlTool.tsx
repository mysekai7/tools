...
type YamlOperation = 'pretty' | 'toJson' | 'fromJson';

export function YamlTool() {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [error, setError] = useState('');
  const { history, addHistory, clearHistory } = useHistory('yaml');
  ...
  const handlePretty = async () => {
    try {
      setError('');
      const result = await window.go.tools.ToolService.YamlPretty(input);
      setOutput(result);
  ...
  const handleToJson = async () => {
    try {
      setError('');
      const result = await window.go.tools.ToolService.YamlToJson(input);
      setOutput(result);
      addHistory(input, result, 'toJson');
    } catch (e) {
      setError('Invalid YAML format');
    }
  };

  const handleFromJson = async () => {
    try {
      setError('');
      const result = await window.go.tools.ToolService.JsonToYaml(input);
      setOutput(result);
      addHistory(input, result, 'fromJson');
    } catch (e) {
  ...
  useKeyboardShortcuts([
    { key: 'y', metaKey: true, shiftKey: true, action: handlePretty, description: 'Pretty YAML (Cmd+Shift+Y)' },
    { key: 'j', metaKey: true, shiftKey: true, action: handleToJson, description: 'YAML to JSON (Cmd+Shift+J)' },
  ...
  return (
    <div className="h-full flex flex-col p-6 bg-white dark:bg-gray-900">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white">YAML Tools</h2>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Format YAML and convert between YAML/JSON
            <span className="ml-2 text-xs text-gray-400 dark:text-gray-500">Cmd+Shift+Y pretty, Cmd+Shift+J to JSON</span>
          </p>
        </div>
        <button
        ...
        <div className={`flex-1 flex flex-col gap-4 ${showHistory ? 'w-2/3' : 'w-full'}`}>
          <div className="flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Input (YAML or JSON)</label>
              <button
                onClick={handlePaste}
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-xs px-1.5 py-0.5 rounded ${
                        ...
                        {item.operation === 'pretty' ? 'pretty' : item.operation === 'toJson' ? 'to JSON' : 'to YAML'}
                      </span>
